...
/*   Updated: 2025/07/06 08:52:47 by le-saad          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void        fill_single_row_pixels(uint32_t *row_arr, mlx_texture_t *texture, size_t row_idx)
{
    size_t              x;
    size_t              base;
    t_pixel  components;
    ...
    {
        base = (row_idx * texture->width * texture->bytes_per_pixel) + (x * texture->bytes_per_pixel);
        components = get_pixel(texture->pixels, base);
        row_arr[x] = ft_pixel(components.r, components.g, components.b, components.a);
        x++;
    }
}
